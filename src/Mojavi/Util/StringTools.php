...
class StringTools {
	...
	static function camelCase($key) {
		return preg_replace_callback("/_([a-zA-Z0-9])/", function($matches) { return strtoupper($matches[1]); }, $key);
	}
...
//		if (intval($orig_screen_width) == 0) { $orig_screen_width = 140; }
	 ...
	 */
	static function LzwCompress($string) {
		// compression
	...
	/**
	 * Finds the ordinal character string for a UTF-8 (16-byte) character
	 * @param string $c
	 ...
	 */
	static function LzwDecompress($binary) {
		$dict = array();
		$data = preg_split('/(?<!^)(?!$)/u', $binary);
		$currChar = $data[0];
		$oldPhrase = $currChar;
		$out = array($currChar);
		$code = 256;
