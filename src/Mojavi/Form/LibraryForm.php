...
/**
 * Provides services to manage libraries loaded in a lib folder
 */
	...
	function queryAll() {
		...
		if (file_exists($this->getFolderName()) && is_dir($this->getFolderName())) {
			...
			foreach ($files as $file) {
				...
				if (substr($file, -10) != '.php') { continue; }
				$library = new LibraryForm();
				$library->setFileName($file);
				$library->setFolderName($this->getFolderName());
				
				$class_name = str_replace('/lib/', '_', str_replace(MO_WEBAPP_DIR . '/lib/', '', $this->getFolderName())) . '_' . str_replace('.php', '', $file);
				
				$library->setClassName($class_name);
				$library->setName(str_replace('.php', '', $file));
				$ret_val[] = $library;
			}
		}
		return $ret_val;
