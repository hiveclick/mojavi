...
class DatabaseResultResource implements Iterator, Countable, ArrayAccess {
	...
	*   MAIN METHODS   *
	...
	function getNextRow() {
		...
		if(is_object($this->getResultResource())) {
			...
			if (($row = $this->getResultResource()->current()) !== false) {
				$this->getResultResource()->next();
				$form = $this->getForm();
	...
	*   GETTERS AND SETTERS   *
	 ...
	 * @return ArrayIterator result resource
	 */
	 ...
	 * @param ArrayIterator result resource $arg0
	 */
	function setResultResource($arg0) {
		if ($arg0 instanceof PDOStatement) {
			$this->result_resource = new ArrayIterator($arg0->fetchAll());
		} else {
			$results = array();
			while (($row = mysql_fetch_assoc($arg0)) !== false) {
				$results[] = $row;
			}
			$this->result_resource = new ArrayIterator($results);
		}
	...
	*	ARRAY ACCESS METHODS   *
	...
	function offsetExists($offset){
		return $this->getResultResource()->offsetExists($offset);
	}
	...
	function offsetGet($offset){
		...
		if($this->offsetExists($offset)) {
			...
			if(is_object($form)) {
				$retval = clone $form;
				$row = $this->getResultResource()->offsetGet($offset);
				$retval->populate($row);
